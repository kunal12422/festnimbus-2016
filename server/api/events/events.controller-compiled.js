...
exports.eventlist = function (req, res, next) {
  event.find({}).select({ 'shortD': 0, 'longD': 0 }).exec(function (err, data) {
    if (err) return next(err);
    var body = {
      'status': 'ok',
      data: data };
...
exports.event_details = function (req, res, next) {
  event.findOne({ '_id': req.params.id }, function (err, event) {
    if (err) return next(err);
    if (!event) console.log("no such event exist");
    var body = {
      'status': 'ok',
...
exports.team_events = function (req, res, next) {
  ...
  if (req.params.team === 'design') {
    res.json({
      'team_events': {
        team: 'Design',
        events: ['KAI PO CHE', 'MODEL MAKING', 'PAPER CRAFT', 'MICRO ART', 'TEMPLE RUN', 'CHEMIQUEST 2.0', 'JACK IT UP', 'HIT ME IF U CAN', 'FUTSAL', 'hokas', 'HADAD', 'QDODAD', 'OIPPII', 'BCHNAMDKAD', 'ADADADDdd', 'ALLDAODAD']
      }
    });
  }
  ...
  if (req.params.team === 'hermatica') {
    res.json({
      'team_events': {
        team: 'Hermatica',
        'events': ['CHEMIQUEST 2.0', 'JACK IT UP', 'HIT ME IF U CAN', 'FUTSAL']
      }
    });
  }
};
...
//# sourceMappingURL=events.controller-compiled.js.map